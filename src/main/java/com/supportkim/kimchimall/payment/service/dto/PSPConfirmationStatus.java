...
public enum PSPConfirmationStatus {
    ...
    public static PSPConfirmationStatus get(String status) {
        for (PSPConfirmationStatus confirmationStatus : values()) {
            if (confirmationStatus.name().equalsIgnoreCase(status)) {
                return confirmationStatus;
            }
        }
        throw new IllegalArgumentException("PSP 승인 상태 (status : " + status + ") 는 올바르지 않은 상태 입니다.");
    }
}
