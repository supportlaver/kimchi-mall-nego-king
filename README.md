...
---
  ...
  #### 2️⃣ 멀티스레드 환경에서 결제 API 호출 시 Dead Lock 문제 발생 - <ins>실행 시간 약 99% 개선</ins>
...
---
  ...
  **문제 상황**
  ...
  - 성능 개선에 많은 방법 중 Tomcat 및 HicariCP 는 요청 처리와 데이터베이스 연결이라는 애플리케이션의 핵심 경로를 담당하기 때문에, 이 경로에서 병목이 발생하면 전체 성능에 영향을 미친다고 판단하여 튜닝을 통해 성능 개선을 하기로 결정
  - 여기부터 시작하기
